    ...
    <div className="bg-white text-neutral-950 overflow-x-hidden">
      ...
      <section className="py-24 px-6 bg-neutral-50">
        <div className="max-w-6xl mx-auto">
          ...
          >
            <p className="text-[#3aaa35] text-xs font-bold tracking-[0.2em] uppercase mb-4">Real Riders. Real Stories.</p>
            <h2 className="text-4xl md:text-5xl font-black mb-6">What DPG Riders Say</h2>
          </motion.div>

          <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
            {[
              { name: "Chisom U.", role: "Delivery Rider, Lagos", text: "I've saved over ₦150,000 in fuel costs since switching to DPG Urban X. The range is perfect for my daily routes, and the motor never lets me down." },
              { name: "Jide K.", role: "Student, Kano", text: "Zero noise, super fast, and turns heads everywhere I go. The Smart Cruiser is the coolest ride on campus. Customer service is top-notch too!" },
              { name: "Amara C.", role: "Business Owner, Abuja", text: "Bought 5 City Volts for our delivery fleet. ROI in 3 months. DPG's support team keeps us running. This is the future of Nigerian logistics." },
            ].map((t, i) => (
              <motion.div
                key={i}
                initial={{ opacity: 0, y: 20 }}
                ...
                className="bg-white p-8 rounded-3xl border border-neutral-200 hover:shadow-lg transition-shadow"
              >
                <div className="flex gap-1 mb-4">
                  {[...Array(5)].map((_, j) => (
                    <div key={j} className="w-4 h-4 bg-[#3aaa35] rounded-full" />
                  ))}
                </div>
                <p className="text-neutral-700 text-base leading-relaxed mb-6">"{t.text}"</p>
                <div>
